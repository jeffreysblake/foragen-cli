...
- Consult [FORA.md](https://github.com/jeffreysblake/foragen-cli/blob/main/FORA.md) (typically found in the project root) for specific instructions related to AI-assisted development, including conventions for React, comments, and Git usage.
...
Container-based sandboxing mounts the project directory (and system temp directory) with read-write access and is started/stopped/removed automatically as you start/stop Gemini CLI. Files created within the sandbox should be automatically mapped to your user/group on host machine. You can easily specify additional mounts, ports, or environment variables by setting `SANDBOX_{MOUNTS,PORTS,ENV}` as needed. You can also fully customize the sandbox for your projects by creating the files `.fora/sandbox.Dockerfile` and/or `.fora/sandbox.bashrc` under your project settings directory (`.fora`) and running `fora` with `BUILD_SANDBOX=1` to trigger building of your custom sandbox.
