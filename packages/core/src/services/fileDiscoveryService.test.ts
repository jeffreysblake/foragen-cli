...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .foraignore patterns even when not in a git repo', async () => {
      await createTestFile('.foraignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);

      expect(service.shouldForaIgnoreFile('secrets.txt')).toBe(true);
      expect(service.shouldForaIgnoreFile('src/index.js')).toBe(false);
    });
  });
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.foraignore', 'logs/');
    });

    it('should filter out git-ignored and fora-ignored files by default', () => {
      const files = [
        'src/index.ts',
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        ...
        respectForaIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectForaIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('shouldGitIgnoreFile & shouldForaIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.foraignore', '*.log');
    });
    ...
    it('should return true for fora-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldForaIgnoreFile(path.join(projectRoot, 'debug.log')),
      ).toBe(true);
    });

    it('should return false for non-fora-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldForaIgnoreFile(path.join(projectRoot, 'src/index.ts')),
      ).toBe(false);
    });
  });
