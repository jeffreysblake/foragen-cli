...
import { type ForaCredentials, type ErrorData } from './foraOAuth2.js';
import type {
...
import { ForaContentGenerator } from './foraContentGenerator.js';
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    private mockCredentials: ForaCredentials | null = null;
    ...
    async getValidCredentials(
      ...
      try {
        const { token } = await foraClient.getAccessToken();
        if (token) {
          const credentials = foraClient.getCredentials();
          return credentials;
        }
      } catch (error) {
        ...
        if (isAuthError) {
          ...
          try {
            const refreshResult = await foraClient.refreshAccessToken();
            if (refreshResult && !('error' in refreshResult)) {
              // Refresh succeeded, update client credentials and return them
              const updatedCredentials = foraClient.getCredentials();
    ...
    setMockCredentials(credentials: ForaCredentials | null): void {
...
describe('ForaContentGenerator', () => {
  let mockForaClient: IForaOAuth2Client;
  let foraContentGenerator: ForaContentGenerator;
  let mockConfig: Config;

  const mockCredentials: ForaCredentials = {
  ...
  beforeEach(() => {
    ...
    // Create ForaContentGenerator instance
    ...
    foraContentGenerator = new ForaContentGenerator(
      mockForaClient,
  ...
  describe('Core Content Generation Methods', () => {
    it('should generate content with valid token', async () => {
      vi.mocked(mockForaClient.getAccessToken).mockResolvedValue({
        token: 'valid-token',
      });
      vi.mocked(mockForaClient.getCredentials).mockReturnValue(mockCredentials);
      ...
      const result = await foraContentGenerator.generateContent(
        request,
        'test-prompt-id',
      );

      expect(result.text).toBe('Generated content');
      expect(mockForaClient.getAccessToken).toHaveBeenCalled();
    });
    ...
    it('should generate content stream with valid token', async () => {
      ...
      const stream = await foraContentGenerator.generateContentStream(
        request,
        'test-prompt-id',
      );
      const chunks: string[] = [];
    ...
    it('should count tokens without requiring authentication', async () => {
      ...
      const result = await foraContentGenerator.countTokens(request);

      expect(result.totalTokens).toBe(15);
      // countTokens is a local operation and should not require OAuth credentials
      expect(mockForaClient.getAccessToken).not.toHaveBeenCalled();
    });
    ...
    it('should embed content with valid token', async () => {
      ...
      const result = await foraContentGenerator.embedContent(request);
  ...
  describe('Token Management and Refresh Logic', () => {
    it('should refresh token on auth error and retry', async () => {
      ...
      vi.mocked(mockForaClient.getAccessToken)
        .mockRejectedValueOnce(authError)
        .mockResolvedValueOnce({ token: 'refreshed-token' });

      // Refresh succeeds
      vi.mocked(mockForaClient.refreshAccessToken).mockResolvedValue({
      ...
      vi.mocked(mockForaClient.getCredentials).mockReturnValue({
        access_token: 'refreshed-token',
        token_type: 'Bearer',
      ...
      expect(result.text).toBe('Generated content');
      expect(mockForaClient.refreshAccessToken).toHaveBeenCalled();
    });
    ...
    it('should handle token refresh failure', async () => {
      ...
      await expect(
        foraContentGenerator.generateContent(request, 'test-prompt-id'),
      ).rejects.toThrow(
        'Failed to obtain valid Fora access token. Please re-authenticate.',
      );
    ...
    it('should update endpoint when token is refreshed', async () => {
      ...
      await foraContentGenerator.generateContent(request, 'test-prompt-id');

      expect(mockForaClient.getCredentials).toHaveBeenCalled();
    });
  });
  ...
  describe('Endpoint URL Normalization', () => {
    it('should use default endpoint when no custom endpoint provided', async () => {
      ...
      const parentPrototype = Object.getPrototypeOf(
        Object.getPrototypeOf(foraContentGenerator),
      );
  ...
  describe('Client State Management', () => {
    it('should set dynamic credentials during operations', async () => {
      const client = (
        foraContentGenerator as unknown as {
  ...
  describe('Error Handling and Retry Logic', () => {
    it('should retry once on authentication errors', async () => {
      ...
      vi.mocked(mockForaClient.getAccessToken).mockImplementation(async () => {
    ...
    it('should not retry non-authentication errors', async () => {
      ...
      expect(mockForaClient.refreshAccessToken).not.toHaveBeenCalled();
    ...
    it('should handle error response from token refresh', async () => {
      vi.mocked(mockForaClient.getAccessToken).mockRejectedValue(
      ...
      await expect(
        foraContentGenerator.generateContent(request, 'test-prompt-id'),
      ).rejects.toThrow('Failed to obtain valid Fora access token');
    });
  });

  describe('Token State Management', () => {
    it('should cache and return current token', () => {
      expect(foraContentGenerator.getCurrentToken()).toBeNull();

      // Simulate setting a token internally
      (
        foraContentGenerator as unknown as { currentToken: string }
      ).currentToken = 'cached-token';

      expect(foraContentGenerator.getCurrentToken()).toBe('cached-token');
    });

    it('should clear token on clearToken()', () => {
      // Simulate having cached token value
      const foraInstance = foraContentGenerator as unknown as {
        currentToken: string;
      };
      foraInstance.currentToken = 'cached-token';

      foraContentGenerator.clearToken();
    ...
    it('should handle concurrent token refresh requests', async () => {
      ...
      vi.mocked(mockForaClient.getAccessToken).mockRejectedValue(authError);
      vi.mocked(mockForaClient.getCredentials).mockReturnValue(mockCredentials);

      vi.mocked(mockForaClient.refreshAccessToken).mockImplementation(
        async () => {
          refreshCallCount++;
  ...
  describe('SharedTokenManager Integration', () => {
    it('should use SharedTokenManager to get valid credentials', async () => {
      ...
      const newGenerator = new ForaContentGenerator(
        mockForaClient,
        { model: 'fora-turbo', authType: AuthType.FORA_OAUTH },
        mockConfig,
      );
  ...
  describe('getCurrentEndpoint Method', () => {
    it('should handle URLs with custom ports', () => {
      ...
      endpoints.forEach(({ input, expected }) => {
        ...
        const generator = foraContentGenerator as unknown as {
  ...
  describe('Stream Error Handling', () => {
    it('should set credentials when stream generation fails', async () => {
      ...
      try {
        await foraContentGenerator.generateContentStream(
          request,
          'test-prompt-id',
        );
      } catch (error) {
  ...
  describe('Constructor and Initialization', () => {
    it('should initialize with configured base URL when provided', () => {
      const generator = new ForaContentGenerator(
        mockForaClient,
        {
          model: 'fora-turbo',
          authType: AuthType.FORA_OAUTH,
  ...
  describe('Edge Cases and Error Conditions', () => {
    ...
    it('should handle method types with token failure (except countTokens)', async () => {
      ...
      await expect(newGenerator.embedContent(embedRequest)).rejects.toThrow(
        'Failed to obtain valid Fora access token',
      );
