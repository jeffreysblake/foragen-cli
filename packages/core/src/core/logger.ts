...
export class Logger {
  private foraDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.foraDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.foraDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.foraDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.foraDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.foraDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.foraDir) {
      console.error(
