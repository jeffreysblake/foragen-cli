...
import type { AnyToolInvocation, Config } from '@jeffreysblake/foragen-cli-core';
import {
  getErrorMessage,
  isNodeError,
  unescapePath,
...
export async function handleAtCommand({
  ...
  for (const atPathPart of atPathCommandParts) {
    ...
    const foraIgnored =
      respectFileIgnore.respectForaIgnore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
        respectGitIgnore: false,
        respectForaIgnore: true,
      });

    if (gitIgnored || foraIgnored) {
      const reason =
        gitIgnored && foraIgnored ? 'both' : gitIgnored ? 'git' : 'fora';
      ignoredByReason[reason].push(pathName);
      const reasonText =
        reason === 'both'
          ? 'ignored by both git and fora'
          : reason === 'git'
            ? 'git-ignored'
            : 'fora-ignored';
  ...
  if (totalIgnored > 0) {
    ...
    if (ignoredByReason['fora'].length) {
      messages.push(`Fora-ignored: ${ignoredByReason['fora'].join(', ')}`);
    }
  ...
  const toolArgs = {
    ...
    file_filtering_options: {
      ...
      respect_fora_ignore: respectFileIgnore.respectForaIgnore,
    },
