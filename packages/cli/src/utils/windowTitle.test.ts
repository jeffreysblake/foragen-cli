...
describe('computeWindowTitle', () => {
  ...
  it('should use default Fora title when CLI_TITLE is not set', () => {
    const result = computeWindowTitle('my-project');
    expect(result).toBe('Fora - my-project');
  });
  ...
  it('should handle folder names with special characters', () => {
    ...
    expect(result).toBe('Fora - project-name_v1.0');
  });
});
