  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        ...
        childProcess.execSync('brew list -1 | grep -q "^foragen-cli$"', {
          stdio: 'ignore',
        });
        return {
    ...
    if (realPath.includes('/.pnpm/global')) {
      const updateCommand = 'pnpm add -g @jeffreysblake/foragen-cli@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add @jeffreysblake/foragen-cli@latest';
      return {
    ...
    if (realPath.includes('/.bun/bin')) {
      const updateCommand = 'bun add -g @jeffreysblake/foragen-cli@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g @jeffreysblake/foragen-cli@latest';
    return {
