...
describe('validateNonInterActiveAuth', () => {
  ...
  it('uses configured FORA_OAUTH if provided', async () => {
    const nonInteractiveConfig = {
      refreshAuth: refreshAuthMock,
      getOutputFormat: vi.fn().mockReturnValue(OutputFormat.TEXT),
      getContentGeneratorConfig: vi
    ...
    expect(refreshAuthMock).toHaveBeenCalledWith(AuthType.FORA_OAUTH);
  });
  ...
  it('exits if currentAuthType does not match enforcedAuthType', async () => {
    mockSettings.merged.security!.auth!.enforcedType = AuthType.FORA_OAUTH;
    process.env['OPENAI_API_KEY'] = 'fake-key';
    const nonInteractiveConfig = {
      refreshAuth: refreshAuthMock,
      getOutputFormat: vi.fn().mockReturnValue(OutputFormat.TEXT),
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'The configured auth type is fora-oauth, but the current auth type is openai. Please re-authenticate with the correct type.',
    );
    expect(processExitSpy).toHaveBeenCalledWith(1);
  });
  ...
  describe('JSON output mode', () => {
    ...
    it('prints JSON error when enforced auth mismatches current auth and exits with code 1', async () => {
      ...
      expect(thrown?.message).toBe('process.exit(1) called');
      {
        ...
        expect(payload.error.code).toBe(1);
        expect(payload.error.message).toContain(
          'The configured auth type is fora-oauth, but the current auth type is openai.',
        );
      }
    });
